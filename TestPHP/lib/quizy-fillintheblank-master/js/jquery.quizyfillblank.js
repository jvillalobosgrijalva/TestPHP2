...
(function($) {
  $.fn.quizyFillBlank = function(options) {
    ...
    var placeHolder = '';
    for(i=0;i<Math.round(opts.blockSize/8);i++){
      placeHolder += '_';
    }
    ...
    function handleDropOn( event, ui ) {
      ...
      var dropId = idAttr.substring(opts.phId.length,idAttr.length);
 	  ui.draggable.attr("data-order",dropId);
      ...
      if(anCount == phNum){
		...
		if(opts.onDropCall!='')
		{
			var divs = $("#"+opts.elementAnId).find("div");
			var values = [];
			for(var i=0; i < divs.length; i++)
			{
				...
				values.push({"id":id, "order": order!=undefined?parseInt(order):1000});
			}
			values = ArrayUtils.orderBy("order", values);
			var string="";
    ...
    for(var i=0; i<phNum; i++){
      el2.append('<label>'+textItems[i]+'</label> <label id="'+
                  opts.phId+i+
      ...
      if(i == phNum-1){
        var nId = i+1;
        el2.append('<label>'+textItems[nId]+'</label>');
      }
    }
    ...
    for(var i=0; i<anNum; i++){
      ...
      elToAppend.append('<div id="'
                        +opts.answerId+i+
                        '" class="form-control m-2 draggable-element '+
                        'quizy-fitb-dropelement" style="width:'+
                        opts.blockSize+'px" data-id="'+ anItemsArr[i].id+'">'+
                        anItemsArr[i].descripcion+'</div>');
  ...
  ****************************************************************************/
  ...
  $.fn.quizyFillBlank.defaults = {elementAnId: 'fillblank-ph', textItems:['Text part1','text part 2', 'text part 3'], elementTextId: 'fillblank-text', anItems: ['an1','an2','an3'], anItemsCorrect:[2,0], answerId:'d-answer', phId: 'd-nest', checkId:'d-check', numberId:'d-number', blockSize:100, onFinishCall:'',onDropCall:'', allowTouchDrag:true}
